...
class Cloth {
  ...
  PVector gravitys = new PVector(0,0.8,0);
  ...
  Cloth() {
    ...
    for (int i = 0; i < cols; i++) {
      ...
      for (int j = 0; j < rows; j++) {
        ...
        if(j > rows-5) {
          cloth[i][j] = new Ball(new PVector(start.x + i*between,start.y + j*between,start.z), new PVector(0,0,0), new PVector(0,0,0), 1);
          vel[i][j] = zeros.copy();
        } else {
          cloth[i][j] = new Ball(new PVector(start.x + i*between,start.y + j*between,start.z), new PVector(0,0,0), new PVector(0,0,0), 0);      
          vel[i][j] = zeros.copy();  
        }
      } 
      //println();
  ...
  void update() {
    ...
    for (int i = 0; i < cols-1; i++) {
      for (int j = 0; j< rows; j++) {
        ...
        //temp.velocity.add(PVector.mult(tem1,dt));
        //tempN.velocity.add(PVector.mult(tem2,dt));
        //temp.velocity.add(PVector.mult(e, f)); 
        //tempN.velocity.sub(PVector.mult(e, f)); // other one
  ...
  void wind() {
    for(int i = 0; i< cols; i++) {  
      for(int j = 0; j<rows; j++) {
        cloth[i][j].velocity.add(new PVector(random(1,2),random(-5,-1),random(-1,0.5)));
      }
    }
  }
  ...
  void drag() {
     for (int i = 0; i < cols -1; i++) {
      for (int j = 0; j< rows -1; j++) {
        ...
        PVector f = PVector.div(PVector.mult(normal,-0.000004 * area * v.magSq()), fac);
        temp.velocity.add(f);
