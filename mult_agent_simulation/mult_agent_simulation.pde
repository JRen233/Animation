...
ArrayList<Boid> boids_oppo = new ArrayList<Boid>(); 
...
void setup() {
  ...
  nextOne = path.size()-2;
  //opposite
  curP_oppo=path.get(0).location.copy();
  nextP_oppo =  path.get(1).location.copy();
  nextOne_oppo = 1;
  ...
  //oppo
  boids_oppo.add(new Boid(new PVector(end.x,end.y)));
  for (int i = 0; i < boids_num; i++) {
    boids_oppo.add(new Boid(new PVector(end.x+i*7,end.y-i*7)));
  }
}
...
void draw() {
  ...
  if (dist(boids.get(10).position.x, boids.get(10).position.y, end.x,end.y) > 10) {
    update();
    oppo_remove();
  } else {
    noLoop();
  ...
  for(int i = 0; i< boids.size(); i++) {
    ...
    //opposite
  for(int i = 0; i< boids_oppo.size(); i++) {
    Boid temp_oppo= boids_oppo.get(i);
    temp_oppo.applyForce(boids_oppo);
  }
  ...
  for(int i = 0; i< boids_oppo.size(); i++) {
    ...
    for(int j = 0; j<obs_list.size(); j++) {
      float d = PVector.dist(obs_list.get(j), temp_oppo.position);
      if(d < (cObsR-22)) {
        PVector diff = PVector.sub(obs_list.get(j), temp_oppo.position);
        diff.normalize();
        diff.mult(3.5);
        temp_oppo.velocity.sub(diff);
      }
    }
    temp_oppo.shape_two();
  }
...
void update() {
  ...
  //opposite
  if (curP_oppo.x == nextP_oppo.x && curP_oppo.y == nextP_oppo.y) {
    if (nextOne_oppo <path.size()-1) {
      nextP_oppo = path.get(nextOne_oppo+1).location.copy();
      nextOne_oppo++;
    }
    
  }
  PVector direction_oppo = PVector.sub(nextP_oppo,curP_oppo);
  direction_oppo.normalize();
  //println(direction, nextP);
  direction_oppo.mult(maxSpeed);
  curP_oppo.add(direction_oppo);
  
  
  if (PVector.dist(curP_oppo,nextP_oppo) < 3) {
    //println("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx");
    curP_oppo = nextP_oppo.copy();
  }
  //println(curP);
  
  // update boids acceleration
  for (int i = 0; i< boids_oppo.size(); i++) {
    PVector dir = PVector.sub(nextP_oppo, boids_oppo.get(i).position);
    dir.normalize();
    boids_oppo.get(i).acceleration.add(dir);
  }
  
}
void oppo_remove(){
  for (Boid b : boids) {
      PVector sepF=b.seperation(boids_oppo);
      b.acceleration.add(sepF.mult(5));
      //b.applyForce(boids_oppo);
    }
  for(Boid bo:boids_oppo){
        PVector sepF=bo.seperation(boids);
      bo.acceleration.add(sepF.mult(5));
      //bo.applyForce(boids_oppo);;
  }
}
